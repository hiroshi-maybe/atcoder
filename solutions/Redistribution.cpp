...
// $ cp-batch Redistribution | diff Redistribution.out -
// $ g++ -std=c++14 -Wall -O2 -D_GLIBCXX_DEBUG -fsanitize=address Redistribution.cpp && ./a.out
...
void solve() {
  ...
  FOR(x,1,S) if(x*3<=S) {
    int rem=S-3*x;
    //dump(x,rem,com.choose(rem+x-1,x-1));
    res+=com.choose(rem+x-1,x-1);
  }
  cout<<res<<endl;
}
