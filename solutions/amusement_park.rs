...
// $ cp-batch amusement_park | diff amusement_park.out -
// $ cargo run --bin amusement_park
...
fn solve() -> i64 {
    let (_n, k) = readln!(usize, i64);
    let a = readln!([i64]);

    let (mut good, mut bad) = (-1, 2_000_000_010i64);
