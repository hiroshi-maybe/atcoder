...
// $ cp-batch knight | diff knight.out -
// $ cargo run --bin knight
...
fn solve() -> u32 {
    ...
    let (s, t) = (2 * y as isize - x as isize, 2 * x as isize - y as isize);
    if s >= 0 && s % 3 == 0 && t >= 0 && t % 3 == 0 {
        let s = s as usize / 3;
        let t = t as usize / 3;
        let com = Com::<ModInt>::new(s + t);
        com.choose(s + t, s).val()
    } else {
        0
    }
}
