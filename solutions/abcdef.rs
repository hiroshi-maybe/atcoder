...
// $ cp-batch abcdef | diff abcdef.out -
// $ cargo run --bin abcdef
...
fn solve() -> u32 {
    let (a, b, c, d, e, f) = readln!(i64, i64, i64, i64, i64, i64);
    (ModInt::from(a) * ModInt::from(b) * ModInt::from(c)
        - ModInt::from(d) * ModInt::from(e) * ModInt::from(f))
    .val()
}
