...
// $ rs-cp-batch big_array | diff big_array.out -
// $ cargo run --bin big_array
...
fn main() {
    ...
    let (n, mut k) = readln!(usize, i64);
    const MAXA: usize = 100_001;
    let mut cnt = vec![0; MAXA];
    for (a, b) in readlns!(usize, i64; n) {
        cnt[a] += b;
    }
