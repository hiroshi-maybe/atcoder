...
// $ cp-batch erase_by_value | diff erase_by_value.out -
// $ cargo run --bin erase_by_value
...
fn solve() -> Vec<i64> {
    ...
    let down = a.windows(2).position(|xs| xs[0] > xs[1]).unwrap_or(n - 1);
    a.iter().filter(|&&x| x != a[down]).copied().collect_vec()
}
