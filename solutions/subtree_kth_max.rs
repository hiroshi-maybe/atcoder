...
// $ cp-batch subtree_kth_max | diff subtree_kth_max.out -
// $ cargo run --bin subtree_kth_max
...
fn dfs(u: usize, par: usize, g: &Vec<Vec<usize>>, x: &Vec<i64>, ks: &mut Vec<Vec<i64>>) {
    let mut top = vec![x[u]];
    for &v in g[u].iter().filter(|&&v| v != par) {
        dfs(v, u, g, x, ks);
        top.extend_from_slice(&ks[v]);
    }
    top.sort_unstable_by_key(|&a| Reverse(a));
    top.truncate(20);
    ks[u] = top;
}
