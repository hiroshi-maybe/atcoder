...
// $ cp-batch multiply_and_rotate | diff multiply_and_rotate.out -
// $ cargo run --bin multiply_and_rotate
...
fn solve() -> isize {
    let (a, n) = readln!(usize, usize);

    let mut d = vec![INF; 1_000_001];
    ...
    while let Some(x) = q.pop_front() {
        ...
        let mut next = vec![];
        if s.len() > 0 && s[0] > 0 {
            let xx = s.iter().fold(0, |ac, d| ac * 10 + d);
            next.push(xx);
        }
        if x % a == 0 {
            next.push(x / a);
