...
// $ cp-batch critical_hit | diff critical_hit.out -
// $ cargo run --bin critical_hit
...
fn solve() -> u32 {
    ...
    let mut dp = vec![ModInt::from(0); n + 10];
    let p2 = ModInt::from(p) / 100;
    let p1 = ModInt::from(1) - p2;
    // dbgln!(p1, p2);

    for n in 2..=n + 1 {
        let cur2 = dp[n - 2];
        let cur1 = dp[n - 1];
        dp[n] = cur2 * p2 + cur1 * p1 + ModInt::from(1);
    }
