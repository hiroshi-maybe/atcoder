...
// $ cp-batch DecayedBridges | diff DecayedBridges.out -
// $ g++ -std=c++14 -Wall -O2 -D_GLIBCXX_DEBUG -fsanitize=address DecayedBridges.cpp && ./a.out
...
void solve() {
  reverse(ALL(es));
  UF uf(N);
  vector<LL> res={1LL*N*(N-1)/2};
  REP(i,M-1) {
    int u,v; tie(u,v)=es[i];
    LL x=res.back();
    if(uf.sameset(u,v)) res.push_back(x);
    else {
      x-=uf.size(u)*uf.size(v);
      res.push_back(x);
      uf.unite(u,v);
    }
  }
