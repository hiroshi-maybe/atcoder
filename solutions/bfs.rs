...
fn solve() -> usize {
    ...
    let s = readlns!([char]; r);
    let g = GridGraph::new(r, c);
    let inf = 100_000;
    let mut d = vvec!(inf;r;c);
    ...
    while let Some(p) = q.pop_front() {
        ...
        let dd = d[i][j];
        //dbgln!(i, j, dd);
        for (ii, jj) in g.adj_cells4(i, j).filter(|&(i, j)| s[i][j] == '.') {
            //dbgln!(ii, jj, d[ii][jj], dd + 1);
            if d[ii][jj] > dd + 1 {
                q.push_back(ii * c + jj);
                d[ii][jj] = dd + 1;
            }
