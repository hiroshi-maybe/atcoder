...
// $ cp-batch sorting_queries | diff sorting_queries.out -
// $ cargo run --bin sorting_queries
...
fn solve() {
    let q = readln!(usize);
    let mut q1 = VecDeque::<i64>::new();
    let mut q2: BinaryHeap<Reverse<i64>> = BinaryHeap::new();
    for _ in 0..q {
        let qs = readln!([i64]);
        match qs[0] {
            1 => {
            ...
            2 => {
                if let Some(Reverse(x)) = q2.pop() {
                    println!("{}", x);
                } else if let Some(x) = q1.pop_front() {
                    println!("{}", x);
                } else {
                    unreachable!()
                }
            ...
            3 => {
                q2.extend(q1.drain(..).map(|x| Reverse(x)));
            }
            _ => unreachable!(),
        };
        //dbgln!(q2, q1);
