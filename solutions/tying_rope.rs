...
// $ cp-batch tying_rope | diff tying_rope.out -
// $ cargo run --bin tying_rope
...
/// 17:26-17:49 AC
///
/// https://atcoder.jp/contests/abc293/editorial/5954
///

fn solve() -> (usize, usize) {
    let (n, m) = readln!(usize, usize);
    let xs = readlns!(usize1, char, usize1, char; m);

    let mut cycle_set = vec![false; 2 * n];
    ...
    for (u, c1, v, c2) in xs {
        let x = 2 * u + (if c1 == 'R' { 0 } else { 1 });
        let y = 2 * v + (if c2 == 'R' { 0 } else { 1 });

        if uf.same_set(x, y) {
            cycle_set[uf.root(x)] = true;
        } else {
            uf.merge(x, y);
        }
    }

    let tot = uf.group_count;
    let cycle_cnt = cycle_set.iter().filter(|&&x| x).count();
    (cycle_cnt, tot - cycle_cnt)
}
