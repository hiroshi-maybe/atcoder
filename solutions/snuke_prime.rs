...
// $ cp-batch snuke_prime | diff snuke_prime.out -
// $ cargo run --bin snuke_prime
...
fn solve() -> i64 {
    let (n, c) = readln!(usize, i64);
    let abc = readlns!(i64, i64, i64; n);

    let mut imos = BTreeMap::new();
    for (l, r, c) in abc {
        *imos.entry(l - 1).or_insert(0) += c;
        *imos.entry(r).or_insert(0) -= c;
    }
