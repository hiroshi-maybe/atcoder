...
// $ cp-batch shrinking | diff shrinking.out -
// $ cargo run --bin shrinking
...
fn solve() -> i64 {
    ...
    for c in b'a'..=b'z' {
        ...
        ps.extend(
            a.iter()
                .enumerate()
                .filter_map(|(i, &cc)| if cc == c { Some(i as i64) } else { None })
                .collect_vec(),
        );
        ps.push(n as i64);

        res.setmin(ps.windows(2).map(|pp| pp[1] - pp[0] - 1).max().unwrap());
    }
