...
// $ cp-batch reordering | diff reordering.out -
// $ cargo run --bin reordering
...
fn solve() -> ModInt {
    ...
    let com = Com::<ModInt>::new(n);
    let mut dp = vvec!(ModInt::from(0); 27; n+1);
    dp[0][0] = ModInt::from(1);
    for c in 0..26 {
        for l in 0..=n {
            let cur = dp[c][l];
            //dbgln!(c, l, cur);
            for delta in (0..=cnt[c]).take_while(|&d| l + d <= n) {
                dp[c + 1][l + delta] += cur * com.choose(l + delta, delta);
            }
        }
    }
