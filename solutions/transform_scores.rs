...
// $ cp-batch transform_scores | diff transform_scores.out -
// $ cargo run --bin transform_scores
...
fn main() {
    ...
    let (n, a, b) = readln!(usize, i64, i64);
    let s = readlns!(i64; n);

    let ma = s.iter().max().unwrap();
    ...
    let p = b as f64 / (ma - mi) as f64;
    let sum = s.iter().sum::<i64>() as f64;
    let q: f64 = a as f64 - p * sum / n as f64;
