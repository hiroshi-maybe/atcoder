...
// $ cp-batch 1111gal_password | diff 1111gal_password.out -
// $ cargo run --bin 1111gal_password
...
fn solve() -> u32 {
    let n = readln!(usize);
    let mut dp = vvec!(ModInt::from(0); n; 11);
    for i in 1..=9 {
        dp[0][i] = ModInt::from(1);
    }
    for i in 0..n - 1 {
        for d in 1..=9 {
            let cur = dp[i][d];
    ...
    dp[n - 1][1..=9].iter().sum::<ModInt>().val()
}
