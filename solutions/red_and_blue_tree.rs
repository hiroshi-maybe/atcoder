...
// $ rs-cp-batch red_and_blue_tree | diff red_and_blue_tree.out -
// $ cargo run --bin red_and_blue_tree
...
fn main() {
    ...
    let (n, _m, k) = readln!(usize, usize, i32);
    let k = k.abs() as usize;
    let a = readln!([usize1]);
    let mut g = vec![vec![]; n];
    for (i, (u, v)) in readlns!(usize1, usize1; n-1).iter().enumerate() {
        g[*u].push((*v, i));
        g[*v].push((*u, i));
    }

    let mut cnt = vec![0; n - 1];
    for es in a.windows(2) {
        let mut res = vec![0; n - 1];

        let u = es[0];
        let v = es[1];
        dfs(u, n + 1, v, &g, &mut res);
        for i in 0..n - 1 {
            cnt[i] += res[i];
        }
    }
    ...
    dbgln!(cnt);
    let mut dp = vvec![ModInt::from(0); 100_001];
    dp[0] = ModInt::from(1);
    for i in 0..n - 1 {
        //dbgln!(dp);
        for k in (0..dp.len()).rev() {
            let x = dp[k];
            if k + cnt[i] < dp.len() {
                dp[k + cnt[i]] += x;
            }
