...
// $ cp-batch SumOfLargeNumbers | diff SumOfLargeNumbers.out -
// $ g++ -std=c++14 -Wall -O2 -D_GLIBCXX_DEBUG -fsanitize=address SumOfLargeNumbers.cpp && ./a.out
...
void solve() {
  ModInt res=0;
  FORE(i,K,N+1) {
    res+=ModInt(i)*N-ModInt(i)*ModInt(i-1)+1;
  }
  cout<<res<<endl;
}
