...
// $ cp-batch increasing_prefix_xor | diff increasing_prefix_xor.out -
// $ cargo run --bin increasing_prefix_xor
...
/// 22:15-22:59 AC
///
/// https://atcoder.jp/contests/arc141/editorial/4022
///
...
fn solve() -> u32 {
    ...
    let n = n as usize;
    let mut dp = vvec!(ModInt::from(0); n+1; 62);
    dp[0][0] = 1.into();
    for i in 0..n {
        for next in 0..=60 {
            let cur = dp[i][next];
            // dbgln!(i, next, cur);
            for j in next..=60 {
                let cnt = min(m + 1, 1 << (j + 1)) - (1 << j);
                dp[i + 1][j + 1] += cur * max(0, cnt);
            }
        }
    }

    dp[n].iter().sum::<ModInt>().val()
}
