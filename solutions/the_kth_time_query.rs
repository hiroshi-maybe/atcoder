...
// $ cp-batch the_kth_time_query | diff the_kth_time_query.out -
// $ cargo run --bin the_kth_time_query
...
fn main() {
    ...
    for (i, &v) in a.iter().enumerate() {
        let e = map.entry(v).or_insert(Vec::new());
        e.push(i as i32);
    }

    for (x, k) in readlns!(i64, usize; q) {
        puts!(
            "{}",
            map.get(&x)
                .and_then(|is| is.get(k - 1).map(|&v| v + 1))
                .unwrap_or(-1)
        );
    }
}
