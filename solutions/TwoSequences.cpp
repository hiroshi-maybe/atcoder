...
/*
 ...
 14:45-15:54 Read editorials and add solution (6WA due to overflow 😞)
 
 Hmmm this is really hard to reduce. There are two operations for A[i] and B[i].
  - summation
  - XOR
 
 I tried to expand (a+b)^(c+d)^... focusing XOR. However it's hard to handle carry in less siginificant bits.
 
 https://twitter.com/rickytheta/status/975009343916490753 is showing detailed process of analysis.
 
 We gonna solve i-th bit independently.
 If we could count number of ON bit at k-th, k-th bit in result could be determined by parity of the count.
 When is k-th bit ON? That happens when A[i]%2^(k+1)+B[k-1..0]%2^(k+1) ranges in [T,2*T) or [3*T,4*T).
 (n[i-1..0] affects 2^i bit in summation)
 Such element of B can be queried by binary search in O(lg N).
 ...
 Key:
  ...
  - Summation => range query works which avoids O(N^2) runtime
 ...
 Summary:
  ...
  - Summation => range query => binary search is typical reduction flow!!
