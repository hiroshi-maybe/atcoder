...
// $ cp-batch king_bombee | diff king_bombee.out -
// $ cargo run --bin king_bombee
...
fn solve() -> u32 {
    let (n, m, k, s, t, x) = readln!(usize, usize, usize, usize1, usize1, usize1);
    let mut g = vec![vec![]; n];
    ...
    let mut dp = vvec!(ModInt::from(0); k+1; 2; n+1);
    dp[0][0][s] = ModInt::from(1);
    for k in 0..k {
        for p in 0..2 {
            for u in 0..n {
                let cur = dp[k][p][u];
                for &v in &g[u] {
                    dp[k + 1][if v == x { 1 - p } else { p }][v] += cur;
                }
            }
        }
    }
