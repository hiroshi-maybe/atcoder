...
// $ cp-batch jumping_takahashi | diff jumping_takahashi.out -
// $ cargo run --bin jumping_takahashi
...
fn solve() -> bool {
    ...
    let mut dp = vec![n + 100; x + 101];
    dp[0] = 0;

    for (i, (a, b)) in readlns!(usize, usize; n).iter().enumerate() {
        for p in (0..=x).rev() {
            if dp[p] == i {
                dp[p + a] = i + 1;
                dp[p + b] = i + 1;
