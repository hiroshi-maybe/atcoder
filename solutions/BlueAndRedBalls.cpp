...
// $ cp-batch BlueAndRedBalls | diff BlueAndRedBalls.out -
// $ g++ -std=c++14 -Wall -O2 -D_GLIBCXX_DEBUG -fsanitize=address BlueAndRedBalls.cpp && ./a.out
...
void solve() {
  ...
  if(N==K) {
    ...
    FORE(i,1,K) {
      if(i==1) res[i-1]=choose(N-K+1,1);
      else {
        ModInt a=choose(N-K+1,i),b=choose(K-1,i-1);
//        dump(i,a,b);
        ModInt c=a*b;
        res[i-1]=c;
      }
