...
// $ cp-batch flipping_signs | diff flipping_signs.out -
// $ cargo run --bin flipping_signs
...
fn main() {
    ...
    let aa = a.iter().map(|x| x.abs()).collect_vec();
    let sum = aa.iter().sum();
    let mina = aa.iter().min().unwrap();
    let par = a.iter().filter(|x| **x < 0).count() % 2;
    let res: i64 = if par == 1 { sum - mina * 2 } else { sum };
