...
// $ cp-batch honestorunkind2 | diff honestorunkind2.out -
// $ cargo run --bin honestorunkind2
...
fn solve() -> u32 {
    ...
    for i in 0..n {
        let a = readln!(usize);
        for _ in 0..a {
            let (x, y) = readln!(usize1, usize);
            xy[i].push((x, if y == 1 { true } else { false }));
        }
    }
    ...
    for mask in 0..1 << n {
        let ok = (0..n)
            .filter(|&i| ith(mask, i))
            .flat_map(|i| &xy[i])
            .all(|&(x, y)| ith(mask, x) == y);
        if ok {
            res.setmax(mask.count_ones());
        }
    }
