...
// $ cp-batch  Interpretation | diff  Interpretation.out -
// $ g++ -std=c++14 -Wall -O2 -D_GLIBCXX_DEBUG -fsanitize=address  Interpretation.cpp && ./a.out
/*
...
void solve() {
  UF uf(N);
  REPE(i,M) REP(j,SZ(L[i])-1) uf.unite(L[i][j],L[i][j+1]);
  cout<<(uf.G==1?"YES":"NO")<<endl;
}
