...
// $ cp-batch crested_ibis_vs_monster | diff crested_ibis_vs_monster.out -
// $ cargo run --bin crested_ibis_vs_monster
...
fn solve() -> usize {
    let (h, n) = readln!(usize, usize);
    let ab = readlns!(usize, usize; n);

    let inf = 100_000_000;
    let mut dp = vec![inf; 20_001];
    dp[0] = 0;
    for h in 0..h {
        for &(a, b) in &ab {
            let cur = dp[h];
