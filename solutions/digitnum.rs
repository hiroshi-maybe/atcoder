...
// $ cp-batch digitnum | diff digitnum.out -
// $ cargo run --bin digitnum
...
fn solve() -> u32 {
    ...
    let mut res = ModInt::from(n) * (ModInt::from(n + 1)) / 2;
    //dbgln!(res);
    let p10 = (0..=18).map(|p| 10i64.pow(p)).collect_vec();
    for d in 0..=17 {
        let cnt = min(n, p10[d + 1] - 1);
        let cnt = max(cnt - p10[d] + 1, 0);
        //dbgln!(d, cnt, p10[d] - 1, cnt * (p10[d] - 1));
        res -= ModInt::from(cnt) * (p10[d] - 1);
    }
