...
// $ cp-batch bridge_and_sheets | diff bridge_and_sheets.out -
// $ cargo run --bin bridge_and_sheets
...
/// 9:06-9:16 AC
///
/// https://atcoder.jp/contests/arc134/editorial/3329
///

fn solve() -> i64 {
    let (_n, l, w) = readln!(usize, i64, i64);
    let mut a = readln!([i64]);
    a.push(l);

    let res = (a[0] + w - 1) / w
        + a.windows(2)
            .map(|xs| (xs[1] - (xs[0] + w) + w - 1) / w)
            .sum::<i64>();
