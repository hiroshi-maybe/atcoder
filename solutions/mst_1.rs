...
// $ cp-batch mst__1 | diff mst__1.out -
// $ cargo run --bin mst__1
...
/// 11:38-12:01 AC
///
/// https://atcoder.jp/contests/abc235/editorial/3254
///

fn solve() -> Vec<bool> {
    let (n, m, q) = readln!(usize, usize, usize);
    let mut es = readlns!(usize1,usize1,i64; m);
    let mut qs = vec![];
    for (i, &(u, v, w)) in readlns!(usize1,usize1,i64; q).iter().enumerate() {
        qs.push((u, v, w, i));
    }

    es.sort_unstable_by_key(|&(_, _, w)| w);
    qs.sort_unstable_by_key(|&(_, _, w, _)| w);
    ...
    for (u, v, w) in es {
        while qi < q && qs[qi].2 < w {
            let (uu, vv, _, i) = qs[qi];
            if !uf.same_set(uu, vv) {
                res[i] = true;
            }
            qi += 1;
        }
