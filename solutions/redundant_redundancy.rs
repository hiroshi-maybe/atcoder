...
// $ cp-batch redundant_redundancy | diff redundant_redundancy.out -
// $ cargo run --bin redundant_redundancy
...
fn solve() -> i64 {
    ...
    let res = 2329089562801;
    //let res = (2..=n).fold(1, |x, acc| lcm(acc, x)) + 1;
    // dbgln!(n, res);

    //assert!((2..=n).all(|x| res % x == 1));
