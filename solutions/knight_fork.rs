...
// $ cp-batch knight_fork | diff knight_fork.out -
// $ cargo run --bin knight_fork
...
fn solve() -> bool {
    ...
    let mut moves = vec![];
    for dx in -2..=2i64 {
        for dy in (-2..=2i64).filter(|&dy| dx.abs() + dy.abs() == 3) {
            moves.push((dx, dy));
        }
    }

    for &m in &moves {
        let (x, y) = (x1 + m.0, y1 + m.1);
        for &mm in &moves {
            let (xx, yy) = (x2 + mm.0, y2 + mm.1);
            if x == xx && y == yy {
                return true;
            }
        }
