...
// $ cp-batch just_one | diff just_one.out -
// $ cargo run --bin just_one
...
fn dfs(u: usize, g: &Vec<Vec<usize>>, viz: &mut Vec<bool>, ucnt: &mut usize, ecnt: &mut usize) {
    if viz[u] {
        return;
    }
    viz[u] = true;
...
fn solve() -> ModInt {
    ...
    for u in 0..n {
        if !viz[u] {
            let (mut ecnt, mut ucnt) = (0, 0);
            dfs(u, &g, &mut viz, &mut ucnt, &mut ecnt);
            if ucnt * 2 == ecnt {
                res *= 2;
            } else {
                res = ModInt::from(0);
