...
// $ rs-cp-batch 123_subtraction | diff 123_subtraction.out -
// $ cargo run --bin 123_subtraction
...
fn main() {
    ...
    let n = readln!(usize);
    let ngs = readlns!(usize; 3);

    const INF: i32 = 1_000_000;
    let mut dp = vvec![INF; n+1];
    dp[n] = if ngs.iter().any(|ng| n == *ng) {
        INF
    } else {
        0
    };
