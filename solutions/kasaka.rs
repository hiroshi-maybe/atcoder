...
// $ cp-batch kasaka | diff kasaka.out -
// $ cargo run --bin kasaka
...
/// 9:18-9:52 AC
///
/// https://atcoder.jp/contests/abc237/editorial/3338
///

fn solve() -> bool {
    let s = readln!([char]);
    let lefta = s.iter().take_while(|&&c| c == 'a').count();
    let righta = s.iter().rev().take_while(|&&c| c == 'a').count();

    let (mut l, mut r) = (lefta as isize, s.len() as isize - righta as isize - 1);
    while l < r {
