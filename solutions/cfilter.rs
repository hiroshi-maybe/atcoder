...
// $ rs-cp-batch cfilter | diff cfilter.out -
// $ cargo run --bin cfilter
...
fn main() {
    ...
    let s = s.trim_end();
    let n = readln!(usize);
    let ts = readlns!([char]; n);

    fn chk(s: &[char], t: &[char]) -> bool {
        t.len() == s.len() && s.iter().zip(t).all(|(&a, &b)| b == '*' || a == b)
    }
