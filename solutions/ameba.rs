...
///
/// 9:30-10:36 give up
/// 10:52 read editorial and got AC
///
/// https://www.slideshare.net/chokudai/arc041
///
...
fn solve() -> Vec<Vec<i32>> {
    ...
    let mut a = a
        .iter()
        .map(|cs| cs.iter().map(|c| (*c as u8 - b'0') as i32).collect_vec())
        .collect_vec();
    let mut res = vvec![0;n;m];
    ...
    for i in 0..n {
        for j in 0..m {
            if a[i][j] > 0 {
                ...
                let i = i + 1;
                for mo in &moves {
                    let ii = i as i32 + mo.0;
                    let jj = j as i32 + mo.1;
                    //dbgln!(i, j, mo, ii, jj);
                    if (0..n as i32).contains(&ii) && (0..m as i32).contains(&jj) {
                        a[ii as usize][jj as usize] -= v;
                    }
                }
            }
            //dbgln!(i, j, a);
