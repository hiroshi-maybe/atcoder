...
// $ cp-batch sugoroku_4 | diff sugoroku_4.out -
// $ cargo run --bin sugoroku_4
...
/// 15:48-16:38 AC
///
/// https://atcoder.jp/contests/abc275/editorial/5116
/// https://atcoder.jp/contests/abc275/submissions/36046163
///

fn f(
    a: usize,
    p: usize,
    memo: &mut HashMap<(usize, usize), ModInt>,
    k: usize,
    m: usize,
    n: usize,
) -> ModInt {
    ...
    if let Some(&res) = memo.get(&(a, p)) {
        return res;
    } else {
        let mut res = ModInt::from(0);
        // dbgln!(a, p);
        for x in 1..=m {
            let back = if p + x > n { p + x - n } else { 0 };
            let nextp = if back > 0 { n - back } else { p + x };
            res += f(a + 1, nextp, memo, k, m, n);
        }
        res /= m;
        memo.insert((a, p), res);
        // dbgln!(a, p, res);
