...
// $ cp-batch range_count_query | diff range_count_query.out -
// $ cargo run --bin range_count_query
...
fn solve() -> Vec<usize> {
    ...
    (0..q)
        .map(|_| {
            let (l, r, x) = readln!(usize1, usize, usize1);
            let rpos = ps[x].lower_bound(&r);
            let lpos = ps[x].lower_bound(&l);
            rpos - lpos
        })
        .collect_vec()
}
