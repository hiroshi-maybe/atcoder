...
// $ cp-batch Ubiquity | diff Ubiquity.out -
// $ g++ -std=c++14 -Wall -O2 -D_GLIBCXX_DEBUG -fsanitize=address Ubiquity.cpp && ./a.out
...
void solve() {
  ModInt res(0);
  FOR(cnt1,1,N) {
    res+=com.choose(N,cnt1)*(ModInt(9).pow(N-cnt1)-ModInt(8).pow(N-cnt1));
  }
  cout<<res<<endl;
}
