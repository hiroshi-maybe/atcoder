...
// $ cp-batch graph_destruction | diff graph_destruction.out -
// $ cargo run --bin graph_destruction
...
/// 22:18-22:38 AC
///
/// https://atcoder.jp/contests/abc229/editorial/2958
///
...
fn solve() -> Vec<usize> {
    ...
    let mut res = (0..n)
        .rev()
        .map(|u| {
            let res = uf.group_count - u - 1;
            for &v in &g[u] {
                uf.merge(u, v);
            }
            //dbgln!(u, uf.group_count);
            res
        })
        .collect_vec();
    res.reverse();
