...
// $ cp-batch distance_sums_2 | diff distance_sums_2.out -
// $ cargo run --bin distance_sums_2
...
fn dfs1(u: usize, par: usize, g: &Vec<Vec<usize>>, cnt: &mut Vec<i64>) -> i64 {
    cnt[u] = 1;
    let mut res = 0;
...
fn dfs2(u: usize, par: usize, g: &Vec<Vec<usize>>, cnt: &Vec<i64>, res: &mut Vec<i64>) {
    let n = g.len();
    if par < n {
        res[u] = res[par] + n as i64 - 2 * cnt[u];
    }
    for &v in g[u].iter().filter(|&&v| v != par) {
        dfs2(v, u, g, cnt, res);
    }
...
fn solve() -> Vec<i64> {
    ...
    let mut cnt = vec![0; n];
    let mut res = vec![0; n];
    res[0] = dfs1(0, n, &g, &mut cnt) - cnt[0];
    dfs2(0, n, &g, &cnt, &mut res);
