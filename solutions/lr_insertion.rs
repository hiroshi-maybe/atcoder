...
// $ cp-batch lr_insertion | diff lr_insertion.out -
// $ cargo run --bin lr_insertion
...
fn solve() -> Vec<usize> {
    ...
    let mut radj = vec![inf; n + 1];
    let mut ladj = vec![inf; n + 1];
	...
	//dbgln!(ladj);
	//dbgln!(radj);
    let mut i = (0..=n).find(|&i| ladj[i] == inf && radj[i] < inf).unwrap();
    let mut res = vec![];
    for _ in 0..=n {
        res.push(i);
        i = radj[i];
