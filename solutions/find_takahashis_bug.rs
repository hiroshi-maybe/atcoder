...
// $ cp-batch find_takahashis_bug | diff find_takahashis_bug.out -
// $ cargo run --bin find_takahashis_bug
...
fn solve() -> bool {
    ...
    let ts = readlns!([usize]; n);
    for mask in 0..k.pow(n as u32) {
        let mut m = mask;
        let mut xor = 0;
        for i in 0..n {
            let j = m % k;
...
fn main() {
    ...
    let res = solve();
    puts!("{}", if res { "Found" } else { "Nothing" });
}
